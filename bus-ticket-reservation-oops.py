...
class passanger(bus):
	def __init__(self):
		self.busname=input("enter the bus name:")
...
class ticket:
		...
		for i in self.bl:
			if p.getbusno()==i.getbus_no(): 
				if i.getbus_qty()>0:
					...
					print("your ticket successfully booked..... make your trip peacefully with ",i.getbus_name())
					self.pl.append(p)
					q=i.getbus_qty()
					p=p.getqty()
					q=q-p
	...
	def get_ticket(self):
		get=input("enter the seatplace like a,b,c,d:")
		for i in self.pl:
			for j in i.getseat():
				if get==j:
					print(">>>>>>>>>> WELCOME  TO ",i.getbusname()," bus <<<<<<<<<<")
	...
	def searchbyplace(self):
			...
			print("   |   BUSNO   |   NAME   |   PLACE  |   QUANTITY")
			for i in self.bl:
	...
	def bus_view(self):
			print("   |   BUSNO   |   NAME   |   PALCE  |   QUANTITY")
			for i in self.bl:
				print("   |   ",i.getbus_no(),"   |   ",i.getbus_name(),"   |   ",i.getbust_raget(),"   |   ",i.getbus_qty())
				print("aviable seat ")
				for j in i.getbut_seat():
...
choose=1
while choose!=0:
	print("************!!!WeLcoMe  to  COIMBATORE ^^^BUS STAND !!!*************  ")
	print("ENTER 1...  IF  YOUR A BUS OWNER FOR ADD BUS...IN BUS STAND  ")
	print("ENTER 2..BOOK THE BUS FOR TRAVEL......")
	print("ENTER 3...VIEW ALL TICKET IN BUSTAND......ITs NOT FOR CUSTOMER")
